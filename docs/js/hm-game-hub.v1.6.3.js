/*
 * Healing Mart Mini Game Hub v1.6.3
 * PC·모바일 반응형 게임 링크 허브
 * 외부 라이브러리 없음
 */
(() => {
  ...
  const CONFIG = Object.freeze({
    ...
    title: "힐링편의점 무료 미니게임",
    subtitle: "퍼즐, 퀴즈, 반응속도, 전략, 랜덤 파티 게임을 검색하고 카테고리별로 찾아보세요.",
    featuredCount: 6,
  ...
  function getInitialSearchQuery() {
    try {
      ...
      return String(params.get("q") || params.get("game") || "").trim();
    } catch (error) {
      return "";
    }
  }
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      @media (max-width:760px) {
        ...
        body:has(#${ROOT_ID}) #sidebar-wrapper,
        body:has(#${ROOT_ID}) .sidebar-wrapper,
        body:has(#${ROOT_ID}) #sidebar,
        body:has(#${ROOT_ID}) .sidebar,
        body:has(#${ROOT_ID}) #blog-sidebar,
        body:has(#${ROOT_ID}) .blog-sidebar,
        body:has(#${ROOT_ID}) .sidebar-area {
          display:none !important;
        }

        body:has(#${ROOT_ID}) #main-wrapper,
        body:has(#${ROOT_ID}) .main-wrapper,
        body:has(#${ROOT_ID}) #content-wrapper,
        body:has(#${ROOT_ID}) .content-wrapper,
        body:has(#${ROOT_ID}) .main-content,
        body:has(#${ROOT_ID}) .main-inner,
        body:has(#${ROOT_ID}) .post-wrapper,
        body:has(#${ROOT_ID}) .post-outer,
        body:has(#${ROOT_ID}) .post-outer-container,
        body:has(#${ROOT_ID}) article.post,
        body:has(#${ROOT_ID}) .post,
        body:has(#${ROOT_ID}) .post-body,
        body:has(#${ROOT_ID}) .entry-content {
          width:100% !important;
        ...
        body:has(#${ROOT_ID}) .entry-content-wrap:has(#${ROOT_ID}),
        body:has(#${ROOT_ID}) .hm-post-main-wrap:has(#${ROOT_ID}) {
          padding:0 !important;
          border:0 !important;
      ...
      @media (max-width:560px) {
        ...
        #${ROOT_ID} .hm-hub-hero {
          ...
          border-radius:22px;
          background:linear-gradient(135deg,#f7fbff 0%,#ffffff 54%,#f8fcff 100%);
        }
  ...
  function syncSearchUrl() {
    try {
      ...
      /* 주소 동기화가 불가능한 환경에서는 검색만 유지합니다. */
    }
  }

  function scheduleSearchUrlSync() {
    window.clearTimeout(scheduleSearchUrlSync.timer);
    scheduleSearchUrlSync.timer = window.setTimeout(syncSearchUrl, 220);
  }
